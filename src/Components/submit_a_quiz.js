...
const QuizAttempter = () => {
 ...
 useEffect(() => {
    const fetchQuiz = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/quiz/${link}`);
        if (!response.ok) {
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/quiz/submit-answers', {
        method: 'POST',
        headers: {
          ...
          {(submit === false) ? (
            <div>
              ...
              <form>
                        ...
                        <li key={option}>
                          <label>
                            <input
                              ...
                              value={option}
                              onChange={() => handleAnswerChange(quiz.questions, question, option)}
                            />
                            {option}
                          </label>
                        </li>
                ...
                <button onClick={(e) => { e.preventDefault(); handleSubmit() }} className='btn'>Submit</button>
              </form>
            </div>
          ) : (
            <div>
              <h1 className='quiz_title'>Quiz Submitted successfully:</h1>
              <button onClick={() => setViewResult(true)} className='btn'>Click to view Result</button>
              {viewResult && (
                <div>
                  <h3 className='title1'>Score: {result.score}</h3>
                  <h3 className='title1'>Percentage: {result.percentageScore}%</h3>
                </div>
              )}
            </div>
          )}
